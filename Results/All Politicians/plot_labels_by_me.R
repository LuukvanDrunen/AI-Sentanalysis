...
df <- read.csv("Bontenbal_labeled_with_dates.csv", sep=';', na.strings = c("", "NA"))
df <- na.omit(df)

df$year_month <- as.Date(format(as.POSIXct(df$date_created, format = "%Y-%m-%dT%H:%M:%S", tz = "UTC"), "%Y-%m-01"))

monthly <- df %>%
  group_by(year_month) %>%
  summarise(
    n_total = n(),
    n_label0 = sum(Labels == 0),
    prop_0 = mean(Labels == 0),
    .groups = 'drop'
  ) %>%
  filter(n_total >= 5) %>%
  filter(year_month >= '2018-04-01')

fit <- loess(prop_0 ~ as.numeric(year_month), data = monthly, span = 0.35)
...
p <- ggplot(monthly, aes(x = year_month)) +
  ...
  geom_col(aes(y = prop_0), fill = "#2166ac", width = 25, alpha = 0.75) +
  
  # Raw proportion dots
  geom_point(aes(y = prop_0, size = n_total),
